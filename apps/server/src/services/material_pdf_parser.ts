...
const extractRyersonMaterialType = (description: string): string | null => {
...
const parseGaugeWallThickness = (segment: string): number | null => {
...
const extractRyersonGaugeOverride = (
  sizeLine: string,
...
const extractRyersonTypeHighlights = (
  description: string,
  type: 'Flat' | 'Round',
): LineHighlight[] => {
  const tokens =
...
const extractRyersonPricingHighlight = (header: string): string | null => {
  const pricingText = header.match(/\$\d+(?:\.\d+)?\s+(?:LB|FT|EA|PC)\b/i)?.[0] ?? null;
  return pricingText;
};

const normalizeRyersonUnitType = (unit: string): 'lb' | 'ft' | 'ea' | '' => {
...
const parseRyersonMaterialDimensions = (
  sizeLine: string,
  description: string,
): {
  type: 'Flat' | 'Round';
