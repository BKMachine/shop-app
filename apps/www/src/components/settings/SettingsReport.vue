<template>
  <v-container>
    ...
    <v-row v-for="(email, index) in emails" :key="email._id" class="mb-0" no-gutters>
      <v-col cols="4">
        <v-text-field v-model="email.email" label="Email" @blur="update(email)" />
      </v-col>
      <v-col class="d-flex row align-center" cols="6">
        <v-checkbox v-model="email.tooling.to" class="ml-2" hide-details style="width: 40px;" />
        <v-checkbox v-model="email.tooling.cc" class="ml-4" hide-details style="width: 40px;" />
      </v-col>
      <v-col class="d-flex align-center" cols="2">
        <v-btn color="error" icon @click="deleteEmail(index)"> <v-icon>mdi-delete</v-icon> </v-btn>
      </v-col>
    </v-row>
    <v-row no-gutters> <v-col cols="12" /> </v-row>
  </v-container>
</template>
