<template>
  <div class="container">
    <v-btn
      v-for="(vendor, i) in vendorStore.sorted"
      :key="vendor._id"
      class="tile elevation-2"
      @click="edit(i)"
    >
      <v-img v-if="vendor.logo" :src="vendor.logo" class="logo"></v-img>
...
const defaultItem: VendorDoc = { _id: '', name: '' };
const editingIndex = ref(-1);
const editingItem = ref<VendorDoc>(defaultItem);
...
function edit(i: number) {
  ...
  editingItem.value = { ...vendorStore.sorted[editingIndex.value] };
  dialog.value = true;
}
